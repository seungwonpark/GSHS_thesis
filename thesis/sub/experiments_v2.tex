...
Two of the following experimental setups - which will be called as `Setup 1' and `Setup 2' respectively - are considered.
Setup 1 is 
%
...
First, we designed a 3D model of \antanospace's skeleton, and printed it with SLA type 3D printer. (Figure \ref{3d_assemblies}) 
Then, two of the load cells were fixed on a stand and connected to the amplifiers. (Figure \ref{anta_loadcell})
Non-elongating wire was used to tightly attach the load cell, \scpnospace, and pulley with a ball bearing and rotary sensor inside. 
Two temperature sensors and plastic tube, was attached to each \scpnospace. 
Each of the tube was connected to solenoid valve and compressed air can via rubber hose. (Figure \ref{anta_tsensor+tube})
Lastly, all of the sensors and muscles of \anta were connected to Arduino Uno with PCB. (Figure \ref{anta_overall})
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.22\textwidth}
		\centering\includegraphics[width=\textwidth]{Anta_loadcell.png}
		\caption{\label{anta_loadcell}}
	\end{subfigure}
	~
	\begin{subfigure}[t]{0.22\textwidth}
		\centering\includegraphics[width=\textwidth]{Anta_Tsensor+tube.png}
		\caption{\label{anta_tsensor+tube}}
	\end{subfigure}
	...
	\end{subfigure}
	\caption[\Anta]{
		\subref{3d_assemblies} The skeleton of the \anta was designed with the software(3D Builder, Microsoft) and 3d-printed.
		\subref{anta_loadcell} Two of the load cells were fixed on a stand.
		\subref{anta_tsensor+tube} Thin plastic tubes that surrounded each \scp was connected to solenoid valve and compressed air can via rubber hose. (The figure shows the utilization of Ecoflex tube instead of plastic tube.)
		\subref{anta_overall} An overall image of the \anta was as above.
	}
	\label{anta_design}
\end{figure}
...
First, we demonstrated open-loop control of cooling speed by trying two-period sinusoidal control. In this demonstration, cooling was done during $t=$ \SI{40}{\second} - \SI{50}{\second}.\footnote{This slightly differs from the Table \ref{table_apc_sustain}.} To cool as much as possible, muscle 2 was cooled for all the time, and muscle 1 was `weakly' cooled by repeatedly opening and closing the solenoid valve. As a result, we got an rms error 26.6\% for the open-loop control, and 6.7\% for the closed-loop control.(Figure \ref{sustain_demo})
...
Integrating \eqref{dynamic_calculation_tau} and \eqref{lambda_control},
we got
...
\end{aligned}
\end{equation}
In order to cool down $ (T_{1}+T_{2})/2 $,
