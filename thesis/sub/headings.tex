...
\noindent{
최근 발견된 super-coiled polymer(SCP) artificial muscle은 기존의 인공근육에 비해 월등한 성능과 낮은 단가를 가져 미래 로봇 산업을 이끌어갈 재료로 주목을 받고 있다. 온도에 반응하는 \scp 을 가열하기 위한 방법은 정립되었으나, 냉각을 위한 효율적인 방법은 세워지지 않았다. 본 논문에서는 \scp 의 특성을 분석하고, 냉각 속도를 feedback 제어하기 위해, 압축공기와 솔레노이드 밸브를 이용하여 열전도도를 제어할 수 있도록 하였다. 또한, 가열을 통해 냉각과 같은 효과를 얻기 위해 \anta 를 제작였으며, 원하는 각도를 따라 6.7\%의 오차로 구동하였다. 최종적으로는 \anta 의 feedback 냉각을 전산모사하여 지속 가능한 \apc 가 가능함을 확인하였다.
}
\end{abstractskor}
