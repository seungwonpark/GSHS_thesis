...
Simulation was carefully done by applying \eqref{thermo-electrical_model} and \eqref{EqAnta}, using the constants we got in section \ref{section_modeling}. The differential equation was numerically solved with 4th Runge-Kutta method. Initial temperature of each muscles are obtained from Fig.\ref{Sinwave_C_T}, thus 

Through many times of simulations, we had concluded that $\alpha = 0.25$ is the best. The result of simulation with this value is shown in Fig.\ref{cool_simulation}. With initial temperature same with Fig.\ref{Sinwave_C_T}, {\it i.e.} $T_{1}=\SI{40}{\degreeCelsius}$ and $T_{2}=\SI{55}{\degreeCelsius}$ at $t=\SI{30}{\second}$, the required thermal conductivity slightly hit the maximum limit.(Fig.\ref{cool_control_lambda}) Also, the average temperature didn't decreased over \SI{30}{\degreeCelsius}.(Fig.\ref{cool_control_T}) Therefore, the temperature will increase with this state.

After average temperature had increased, it will be easier to cool the muscle because $dT/dt \propto (T-T_{amb})$. Simulation was done again with initial temperature $T_{1}=\SI{85}{\degreeCelsius}$ and $T_{2}=\SI{100}{\degreeCelsius}$ at $t=\SI{30}{\second}$. In this time, there was dramatical decrease of average temperature. (Fig.\ref{cool_control_T_100}) Also, required thermal conductivity had hit the limit after cooling two of the muscle enough, as shown in Fig.\ref{cool_control_lambda_100}. After reaching the wanted amount of cooling, we can maintain $\theta_{ref}$ by switching to heating. Finally, we can conclude that feedback cooling control is possible, so that we can make \apc to be highly sustainable.

\begin{figure}[t]
	\centering
	\begin{subfigure}[t]{0.45\textwidth}
		\centering\includegraphics[width=\textwidth]{cool_control_T_v3.pdf}
		\caption{\label{cool_control_T}}
	\end{subfigure}%
	\begin{subfigure}[t]{0.45\textwidth}
		\centering\includegraphics[width=\textwidth]{cool_control_lambda_v3.pdf}
		\caption{\label{cool_control_lambda}}
	\end{subfigure}
	
	
	\begin{subfigure}[t]{0.45\textwidth}
		\centering\includegraphics[width=\textwidth]{cool_control_T_v3_100_85.pdf}
		\caption{\label{cool_control_T_100}}
	\end{subfigure}%
	\begin{subfigure}[t]{0.45\textwidth}
		\centering\includegraphics[width=\textwidth]{cool_control_lambda_v3_100_85.pdf}
		\caption{\label{cool_control_lambda_100}}
	\end{subfigure}
	\caption[Sustainable closed-loop \apc simulation]{\subref{cool_control_T} Temperature of two muscles in function of time after $t=\SI{30}{\second}$. \subref{cool_control_lambda} Thermal conductivity of two muscles in function of time. \subref{cool_control_T_100} \subref{cool_control_lambda_100} Same simulation was done with different initial temperature.}
